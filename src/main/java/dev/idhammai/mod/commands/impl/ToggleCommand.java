...
extends Command {
    ...
    public void runCommand(String[] parameters) {
        ...
        Module module = suncat.MODULE.getModuleByName(moduleName);
        if (module == null) {
    ...
    public String[] getAutocorrect(int count, List<String> seperated) {
        if (count == 1) {
            ...
            for (Module x : suncat.MODULE.getModules()) {
                if (!input.equalsIgnoreCase(suncat.getPrefix() + "toggle") && !x.getName().toLowerCase().startsWith(input)) continue;
