...
extends Screen {
    ...
    private void renderSearchBar(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        int searchBarHeight = ClickGui.getInstance().searchBarHeight.getValueInt();
        int screenWidth = this.width;
        int searchBarWidth = Math.min(200, screenWidth - 40);
        int searchBarX = (screenWidth - searchBarWidth) / 2;
        int searchBarY = 28; // 在顶部标签页下方
        ...
        if (isLiquid && isLiquidGlass) {
            ...
            Render2DUtil.drawRoundedRect(context.getMatrices(), (float)searchBarX, (float)searchBarY, (float)searchBarWidth, (float)searchBarHeight, 6.0f, bgColor);
            ...
            Render2DUtil.drawGradientRoundedRect(context.getMatrices(), (float)searchBarX + 2, (float)searchBarY + 1, (float)searchBarWidth - 4, (float)searchBarHeight / 2.5f, 3.0f, highlightTop, highlightBottom);
            
            // 边缘
            Render2DUtil.drawRoundedStroke(context.getMatrices(), (float)searchBarX, (float)searchBarY, (float)searchBarWidth, (float)searchBarHeight, 6.0f, new Color(100, 130, 150, 50), 16);
        } else {
            // 普通风格
            Color bgColor = new Color(30, 30, 30, 200);
            Render2DUtil.drawRoundedRect(context.getMatrices(), (float)searchBarX, (float)searchBarY, (float)searchBarWidth, (float)searchBarHeight, 6.0f, bgColor);
            Render2DUtil.drawRoundedStroke(context.getMatrices(), (float)searchBarX, (float)searchBarY, (float)searchBarWidth, (float)searchBarHeight, 6.0f, this.searchFocused ? ClickGui.getInstance().getColor(0.0) : new Color(80, 80, 80, 150), 16);
        }
        
        // 绘制搜索图标（🔍）
        String searchIcon = "🔍";
        int iconX = searchBarX + 8;
        int iconY = searchBarY + (searchBarHeight - 9) / 2;
        if (FontManager.isCustomFontEnabled()) {
        ...
        // 绘制搜索文本或提示文字
        int textX = searchBarX + 24;
        int textY = searchBarY + (searchBarHeight - 9) / 2;
        ...
        if (mouseX >= searchBarX && mouseX <= searchBarX + searchBarWidth && mouseY >= searchBarY && mouseY <= searchBarY + searchBarHeight) {
            // 悬停时绘制高亮边框
            Render2DUtil.drawRoundedStroke(context.getMatrices(), (float)searchBarX, (float)searchBarY, (float)searchBarWidth, (float)searchBarHeight, 6.0f, new Color(120, 150, 180, 100), 16);
        }
    }
