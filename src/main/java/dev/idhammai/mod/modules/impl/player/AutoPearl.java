...
extends Module {
    ...
    public void onUpdate(TickEvent event) {
        ...
        if (!this.shouldYawStep()) {
            this.throwPearl(AutoPearl.mc.player.getYaw(), AutoPearl.mc.player.getPitch());
            this.disable();
        } else if (suncat.ROTATION.inFov(AutoPearl.mc.player.getYaw(), AutoPearl.mc.player.getPitch(), this.fov.getValueFloat())) {
            if (this.sync.getValue()) {
                ...
                if (AutoPearl.mc.player.getMainHandStack().getItem() == Items.ENDER_PEARL) {
                    AutoPearl.sendSequencedPacket(id -> new PlayerInteractItemC2SPacket(Hand.MAIN_HAND, id, suncat.ROTATION.getLastYaw(), suncat.ROTATION.getLastPitch()));
