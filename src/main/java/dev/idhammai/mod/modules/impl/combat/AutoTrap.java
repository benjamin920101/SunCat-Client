...
extends Module {
    ...
    private void trapTarget(PlayerEntity target) {
        if (this.onlyHole.getValue() && !suncat.HOLE.isHole(EntityUtil.getEntityPos((Entity)target))) {
            return;
        }
    ...
    private void doTrap(PlayerEntity player, BlockPos pos) {
        ...
        if (this.antiStep.getValue() && (suncat.BREAK.isMining(pos.up(headOffset)) || !this.onlyBreak.getValue())) {
        ...
        if (this.chestUp.getValue()) {
            for (Direction i : Direction.values()) {
                ...
                if (this.onlyBreaking.getValue() && !suncat.BREAK.isMining(pos.up(headOffset))) continue;
    ...
    public BlockPos getHelper(BlockPos pos) {
        ...
        for (Direction i : Direction.values()) {
            if (this.checkMine.getValue() && suncat.BREAK.isMining(pos.offset(i)) || !BlockUtil.isStrictDirection(pos.offset(i), i.getOpposite()) || !BlockUtil.canPlace(pos.offset(i), this.placeRange.getValue(), this.breakCrystal.getValue())) continue;
            return pos.offset(i);
        }
        return null;
    }
    ...
    public BlockPos getHelper(BlockPos pos, Direction ignore) {
        ...
        for (Direction i : Direction.values()) {
            if (i == ignore || this.checkMine.getValue() && suncat.BREAK.isMining(pos.offset(i)) || !BlockUtil.isStrictDirection(pos.offset(i), i.getOpposite()) || !BlockUtil.canPlace(pos.offset(i), this.placeRange.getValue(), this.breakCrystal.getValue())) continue;
            return pos.offset(i);
        }
        return null;
    }
    ...
    private boolean checkEntity(BlockPos pos) {
        ...
        for (Entity entity : suncat.THREAD.getPlayers()) {
