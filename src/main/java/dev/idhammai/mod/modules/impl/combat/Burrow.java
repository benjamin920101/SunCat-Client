...
extends Module {
    ...
    public void onUpdate(ClientTickEvent event) {
        ...
        if (event.isPost()) {
            ...
            if (suncat.PLAYER.isInWeb((PlayerEntity)Burrow.mc.player)) {
                this.webTimer.reset();
                return;
            }
            ...
            if (this.rotate.getValue() == RotateMode.Bypass) {
                if (above) {
                    ...
                    suncat.ROTATION.snapAt(angle[0], angle[1]);
                } else {
    ...
    private boolean canPlace(BlockPos pos) {
        ...
        if (this.detectMine.getValue() && suncat.BREAK.isMining(pos)) {
            return false;
        }
        return !this.hasEntity(pos);
    }

    // Public method for BurrowAssist
    public boolean canPlacePublic(BlockPos pos) {
        if (this.noSelfPos.getValue() && pos.equals((Object)EntityUtil.getPlayerPos(true))) {
            return false;
        }
        if (!BlockUtil.allowAirPlace() && BlockUtil.getPlaceSide(pos) == null) {
