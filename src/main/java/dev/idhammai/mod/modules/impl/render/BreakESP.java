...
extends Module {
    ...
    private Color getFillColor(PlayerEntity player) {
        return suncat.FRIEND.isFriend(player) ? this.fillFriend.getValue() : this.fill.getValue();
    }

    private Color getBoxColor(PlayerEntity player) {
        return suncat.FRIEND.isFriend(player) ? this.boxFriend.getValue() : this.box.getValue();
    }

    @Override
    public void onRender3D(MatrixStack matrixStack) {
        for (BreakManager.BreakData breakData : suncat.BREAK.breakMap.values()) {
        ...
        if (this.second.getValue()) {
            Iterator<BreakManager.BreakData> iterator = ((ConcurrentHashMap.KeySetView)suncat.BREAK.doubleMap.keySet()).iterator();
            while (iterator.hasNext()) {
                int i = (Integer)((Object)iterator.next());
                BreakManager.BreakData breakData = suncat.BREAK.doubleMap.get(i);
                if (breakData == null || breakData.getEntity() == null || BreakESP.mc.world.isAir(breakData.pos)) {
                    suncat.BREAK.doubleMap.remove(i);
                    continue;
                }
                BreakManager.BreakData singleBreakData = suncat.BREAK.breakMap.get(i);
