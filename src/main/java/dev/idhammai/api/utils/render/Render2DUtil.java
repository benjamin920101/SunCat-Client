...
implements Wrapper {
    ...
    public static void drawGradientRoundedRect(MatrixStack matrices, float x, float y, float width, float height, float radius, Color topColor, Color bottomColor) {
        if (width <= 0.0f || height <= 0.0f) {
            return;
        }
        if (radius <= 0.0f) {
            Render2DUtil.drawRect(matrices, x, y, width, height, topColor);
            return;
        }
        float r = Math.min(radius, Math.min(width, height) / 2.0f);
        int seg = 16;
        ...
        // 顶部
        buffer.vertex(matrix, x + r, y + halfH, 0.0f).color(topColor.getRGB());
        buffer.vertex(matrix, x + r, y, 0.0f).color(topColor.getRGB());
        buffer.vertex(matrix, x + width - r, y + halfH, 0.0f).color(topColor.getRGB());
        buffer.vertex(matrix, x + width - r, y, 0.0f).color(topColor.getRGB());
        // 中部渐变
        buffer.vertex(matrix, x + r, y + halfH, 0.0f).color(topColor.getRGB());
        buffer.vertex(matrix, x + r, y + height - halfH, 0.0f).color(bottomColor.getRGB());
        buffer.vertex(matrix, x + width - r, y + halfH, 0.0f).color(topColor.getRGB());
        buffer.vertex(matrix, x + width - r, y + height - halfH, 0.0f).color(bottomColor.getRGB());
        // 底部
        buffer.vertex(matrix, x + r, y + height - halfH, 0.0f).color(bottomColor.getRGB());
        buffer.vertex(matrix, x + r, y + height, 0.0f).color(bottomColor.getRGB());
        buffer.vertex(matrix, x + width - r, y + height - halfH, 0.0f).color(bottomColor.getRGB());
        buffer.vertex(matrix, x + width - r, y + height, 0.0f).color(bottomColor.getRGB());
        BufferRenderer.drawWithGlobalProgram((BuiltBuffer)buffer.end());
        // 左上圆角
        buffer = tessellator.begin(VertexFormat.DrawMode.TRIANGLE_FAN, VertexFormats.POSITION_COLOR);
        buffer.vertex(matrix, x + r, y + r, 0.0f).color(topColor.getRGB());
        for (int i = 0; i <= seg; ++i) {
            double angle = Math.PI + (double)i * 1.5707963267948966 / (double)seg;
            float px = x + r + (float)(Math.cos(angle) * (double)r);
            float py = y + r + (float)(Math.sin(angle) * (double)r);
            buffer.vertex(matrix, px, py, 0.0f).color(topColor.getRGB());
        }
        BufferRenderer.drawWithGlobalProgram((BuiltBuffer)buffer.end());
        // 右上圆角
        buffer = tessellator.begin(VertexFormat.DrawMode.TRIANGLE_FAN, VertexFormats.POSITION_COLOR);
        buffer.vertex(matrix, x + width - r, y + r, 0.0f).color(topColor.getRGB());
        for (int i = 0; i <= seg; ++i) {
            double angle = 1.5707963267948966 + (double)i * 1.5707963267948966 / (double)seg;
            float px = x + width - r + (float)(Math.cos(angle) * (double)r);
            float py = y + r + (float)(Math.sin(angle) * (double)r);
            buffer.vertex(matrix, px, py, 0.0f).color(topColor.getRGB());
        }
        BufferRenderer.drawWithGlobalProgram((BuiltBuffer)buffer.end());
        // 右下圆角
        buffer = tessellator.begin(VertexFormat.DrawMode.TRIANGLE_FAN, VertexFormats.POSITION_COLOR);
        buffer.vertex(matrix, x + width - r, y + height - r, 0.0f).color(bottomColor.getRGB());
        for (int i = 0; i <= seg; ++i) {
            double angle = 0.0 + (double)i * 1.5707963267948966 / (double)seg;
            float px = x + width - r + (float)(Math.cos(angle) * (double)r);
            float py = y + height - r + (float)(Math.sin(angle) * (double)r);
            buffer.vertex(matrix, px, py, 0.0f).color(bottomColor.getRGB());
        }
        BufferRenderer.drawWithGlobalProgram((BuiltBuffer)buffer.end());
        // 左下圆角
        buffer = tessellator.begin(VertexFormat.DrawMode.TRIANGLE_FAN, VertexFormats.POSITION_COLOR);
        buffer.vertex(matrix, x + r, y + height - r, 0.0f).color(bottomColor.getRGB());
        for (int i = 0; i <= seg; ++i) {
            double angle = 4.71238898038469 + (double)i * 1.5707963267948966 / (double)seg;
            float px = x + r + (float)(Math.cos(angle) * (double)r);
            float py = y + height - r + (float)(Math.sin(angle) * (double)r);
            buffer.vertex(matrix, px, py, 0.0f).color(bottomColor.getRGB());
        }
