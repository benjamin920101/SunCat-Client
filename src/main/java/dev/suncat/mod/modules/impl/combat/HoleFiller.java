...
extends Module {
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        CombatUtil.getEnemies(this.enemyRange.getValue()).stream().flatMap(enemy -> BlockUtil.getSphere(this.holeRange.getValueFloat(), PredictUtil.getPos(enemy, this.predictTicks.getValueInt())).stream()).filter(pos -> pos.toCenterPos().distanceTo(HoleFiller.mc.player.getPos()) > this.selfRange.getValue() && (suncat.HOLE.isHole((BlockPos)pos, true, true, false) || suncat.HOLE.isDoubleHole((BlockPos)pos))).distinct().forEach(this::tryPlaceBlock);
    }
