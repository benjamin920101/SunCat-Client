...
extends Module {
    ...
    private void openGui() {
        ...
        if (!Sorter.nullCheck()) {
            mc.setScreen((Screen)new WindowsScreen(new ItemSelectWindow(suncat.CLEANER)));
        }
    }
    ...
    private boolean shouldDrop(ItemStack stack) {
        ...
        if (this.trash.getValue() && (!(item instanceof BlockItem) || !((blockItem = (BlockItem)item).getBlock() instanceof ShulkerBoxBlock)) && (!(inList = suncat.CLEANER.inList(item.getTranslationKey())) && this.trashMode.is(Mode.Whitelist) || inList && this.trashMode.is(Mode.Blacklist))) {
            return true;
        }
