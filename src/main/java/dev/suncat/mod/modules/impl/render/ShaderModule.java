...
import dev.suncat.asm.accessors.IGameRenderer;
...
extends Module {
    ...
    public boolean shouldRender(Entity entity) {
        ...
        if (entity instanceof PlayerEntity) {
            ...
            if (suncat.FRIEND.isFriend(playerEntity)) {
    ...
    public void onRender3D(MatrixStack matrixStack) {
        if (this.hands.getValue()) {
            suncat.SHADER.renderShader(() -> ((IGameRenderer)ShaderModule.mc.gameRenderer).IRenderHand(ShaderModule.mc.gameRenderer.getCamera(), mc.getRenderTickCounter().getTickDelta(true), matrixStack.peek().getPositionMatrix()), this.mode.getValue());
        }
    }

    @Override
    public void onToggle() {
        suncat.SHADER.reloadShaders();
    }
