...
extends Module {
    ...
    public static void pistonFacing(Direction i) {
        if (i == Direction.EAST) {
            suncat.ROTATION.snapAt(-90.0f, 5.0f);
        } else if (i == Direction.WEST) {
            suncat.ROTATION.snapAt(90.0f, 5.0f);
        } else if (i == Direction.NORTH) {
            suncat.ROTATION.snapAt(180.0f, 5.0f);
        } else if (i == Direction.SOUTH) {
            suncat.ROTATION.snapAt(0.0f, 5.0f);
        }
    }
        ...
        if (this.isTrueFacing(piston, direction) && this.facingCheck(piston) && BlockUtil.clientCanPlace(piston, false)) {
            ...
            if (BlockUtil.getPlaceSide(piston, this.placeRange.getValue()) != null) {
                ...
                if (canPower) {
                    ...
                    if (side != null) {
                        if (this.rotate.getValue()) {
                            suncat.ROTATION.lookAt(piston.offset(side), side.getOpposite());
                        }
                ...
                if (powerFacing != null) {
                    ...
                    if (side2 != null) {
                        if (this.rotate.getValue()) {
                            suncat.ROTATION.lookAt(piston.offset(side2), side2.getOpposite());
                        }
                        if (this.yawDeceive.getValue()) {
                            AutoPush.pistonFacing(direction.getOpposite());
                        }
    ...
    private Boolean canPush(PlayerEntity player) {
        ...
        if (!this.allowWeb.getValue() && suncat.PLAYER.isInWeb(player)) {
            return false;
        }
        ...
        return (double)progress > this.surroundCheck.getValue() - 1.0 || suncat.HOLE.isHard(new BlockPosX(player.getX(), player.getY() + 0.5, player.getZ()));
    }
