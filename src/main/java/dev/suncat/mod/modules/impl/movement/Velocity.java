...
extends Module {
    ...
    public void onRotate(UpdateRotateEvent event) {
        if (this.noRotation()) {
            event.setRotation(suncat.ROTATION.rotationYaw, 89.0f);
        }
    }
    ...
    public void onUpdate(TickEvent event) {
        ...
        if (this.flag) {
            if (this.lagBackTimer.passedMs(this.lagPause.getValue()) && (this.flagInWall.getValue() && (!pushOutOfBlocks || this.whenPushOutOfBlocks.getValue()) || !EntityUtil.isInsideBlock())) {
                mc.getNetworkHandler().sendPacket((Packet)new PlayerMoveC2SPacket.Full(Velocity.mc.player.getX(), Velocity.mc.player.getY(), Velocity.mc.player.getZ(), suncat.ROTATION.rotationYaw, suncat.ROTATION.rotationPitch, Velocity.mc.player.isOnGround()));
