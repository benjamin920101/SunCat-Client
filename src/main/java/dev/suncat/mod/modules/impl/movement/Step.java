...
import dev.suncat.mod.modules.impl.combat.SelfTrap;
import dev.suncat.mod.modules.impl.combat.Surround;
...
extends Module {
    ...
    public void onUpdate(UpdateEvent event) {
        if (this.pathingPause.getValue() && BaritoneUtil.isActive() || this.sneakingPause.getValue() && Step.mc.player.isSneaking() || this.inBlockPause.getValue() && EntityUtil.isInsideBlock() || Step.mc.player.isInLava() || Step.mc.player.isTouchingWater() || this.inWebPause.getValue() && suncat.PLAYER.isInWeb((PlayerEntity)Step.mc.player) || !Step.mc.player.isOnGround() || this.onlyMoving.getValue() && !MovementUtil.isMoving() || this.surroundPause.getValue() && (Surround.INSTANCE.isOn() || SelfTrap.INSTANCE.isOn())) {
            Step.setStepHeight(0.6f);
            return;
        }
        ...
        if (this.mode.getValue().equals((Object)Mode.OldNCP) || strict) {
            ...
            if (offsets != null && offsets.length > 1) {
                if (this.useTimer.getValue()) {
                    suncat.TIMER.set((float)this.getTimer(stepHeight));
                    this.timer = true;
                    this.packets = 2;
                }
