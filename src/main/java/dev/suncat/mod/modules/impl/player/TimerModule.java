    ...
    public void onRender2D(DrawContext drawContext, float tickDelta) {
        ...
        if (MovementUtil.isMoving() && !EntityUtil.isInsideBlock()) {
            ...
            if (this.timer2.passed(this.lastMs)) {
                ...
                double timer = (double)suncat.TIMER.getDefault() + (1.0 - this.end.ease(this.ease.getValue())) * (double)(this.shiftTimer.getValueFloat() - 1.0f) * ((double)this.lastMs / this.accumulate.getValue());
                suncat.TIMER.set((float)Math.max((double)suncat.TIMER.getDefault(), timer));
            } else {
                suncat.TIMER.set(this.shiftTimer.getValueFloat());
            }
        } else {
            if (this.moving) {
                suncat.TIMER.reset();
