...
import dev.suncat.api.utils.render.ModelPlayer;
...
extends Module {
    ...
    public void onRender3D(MatrixStack matrixStack) {
        for (ModelPlayer data : this.logoutCache.values()) {
            ...
            Render3DUtil.drawText3D(player.getName().getString() + (String)(this.health.getValue() ? String.valueOf(TextRadar.getHealthColor(player)) + " " + LogoutSpots.round2(player.getHealth() + player.getAbsorptionAmount()) : "") + (String)(this.totem.getValue() && suncat.POP.getPop(player) > 0 ? String.valueOf(TextRadar.getPopColor(suncat.POP.getPop(player))) + " -" + suncat.POP.getPop(player) : ""), new Vec3d(player.getX(), ((IEntity)player).getDimensions().getBoxAt((Vec3d)player.getPos()).maxY + 0.5, player.getZ()), this.text.getValue());
        }
    }
