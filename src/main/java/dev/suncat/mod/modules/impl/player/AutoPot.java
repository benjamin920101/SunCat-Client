...
extends Module {
    ...
    public AutoPot() {
        ...
        INSTANCE = this;
        suncat.EVENT_BUS.subscribe(new AutoPotTick());
    }
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        if (!this.onlyGround.getValue() || (AutoPot.mc.player.isOnGround() || suncat.PLAYER.isInWeb((PlayerEntity)AutoPot.mc.player)) && !AutoPot.mc.world.isAir((BlockPos)new BlockPosX(AutoPot.mc.player.getPos().add(0.0, -1.0, 0.0)))) {
    ...
    public void throwPotion(StatusEffect targetEffect) {
        ...
        if (this.inventory.getValue() && (newSlot = AutoPot.findPotionInventorySlot(targetEffect)) != -1) {
            suncat.ROTATION.snapAt(suncat.ROTATION.rotationYaw, 90.0f);
            InventoryUtil.inventorySwap(newSlot, AutoPot.mc.player.getInventory().selectedSlot);
            AutoPot.sendSequencedPacket(id -> new PlayerInteractItemC2SPacket(Hand.MAIN_HAND, id, suncat.ROTATION.getLastYaw(), suncat.ROTATION.getLastPitch()));
            InventoryUtil.inventorySwap(newSlot, AutoPot.mc.player.getInventory().selectedSlot);
