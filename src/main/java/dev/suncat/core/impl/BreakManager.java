...
import dev.suncat.api.events.impl.BlockBreakingProgressEvent;
import dev.suncat.api.events.impl.ClientTickEvent;
import dev.suncat.api.events.impl.PacketEvent;
import dev.suncat.api.events.impl.ServerConnectBeginEvent;
...
import dev.suncat.mod.modules.impl.render.BreakESP;
...
implements Wrapper {
    ...
    public void onTick(ClientTickEvent event) {
        ...
        if (AntiCheat.INSTANCE.detectDouble.getValue()) {
            Iterator<Object> iterator = ((ConcurrentHashMap.KeySetView)suncat.BREAK.doubleMap.keySet()).iterator();
            while (iterator.hasNext()) {
                int i = (Integer)iterator.next();
                BreakData breakData = suncat.BREAK.doubleMap.get(i);
    ...
    public void onPacket(PacketEvent.Receive event) {
        ...
        if (packet instanceof BlockBreakingProgressS2CPacket) {
            ...
            suncat.EVENT_BUS.post(BlockBreakingProgressEvent.get(packet2.getPos(), packet2.getEntityId(), packet2.getProgress()));
