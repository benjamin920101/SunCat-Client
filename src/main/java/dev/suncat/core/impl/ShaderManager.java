...
import dev.suncat.api.interfaces.IShaderEffectHook;
...
implements Wrapper {
    ...
    public void applyShader(Runnable runnable, Shader mode) {
        ...
        if (effect != null) {
            ((IShaderEffectHook)effect).suncatClient$addHook("bufIn", this.shaderBuffer);
        } else {
            return;
        }
    ...
    public void reloadShaders() {
        ...
        DEFAULT_OUTLINE = ShaderEffectManager.getInstance().manage(Identifier.of((String)"shaders/post/outline.json"), managedShaderEffect -> {
            ...
            ((IShaderEffectHook)effect).suncatClient$addHook("bufIn", ShaderManager.mc.worldRenderer.getEntityOutlinesFramebuffer());
            ((IShaderEffectHook)effect).suncatClient$addHook("bufOut", ShaderManager.mc.worldRenderer.getEntityOutlinesFramebuffer());
        });
