...
import dev.suncat.mod.modules.impl.combat.Breaker;
...
implements Wrapper {
    ...
    public boolean isHole(BlockPos pos, boolean canStand, boolean checkTrap, boolean anyBlock) {
        ...
        for (Direction i : Direction.values()) {
            if (i == Direction.UP || i == Direction.DOWN || (!anyBlock || HoleManager.mc.world.isAir(pos.offset(i))) && !suncat.HOLE.isHard(pos.offset(i))) continue;
            ++blockProgress;
        }
    ...
    public BlockPos getHole(float range, boolean doubleHole, boolean any, boolean up) {
        ...
        for (BlockPos pos : BlockUtil.getSphere(range, HoleManager.mc.player.getPos())) {
            if ((pos.getX() != HoleManager.mc.player.getBlockX() || pos.getZ() != HoleManager.mc.player.getBlockZ()) && !up && (double)(pos.getY() + 1) > HoleManager.mc.player.getY() || !suncat.HOLE.isHole(pos, true, true, any) && (!doubleHole || !this.isDoubleHole(pos)) || pos.getY() - HoleManager.mc.player.getBlockY() > 1) continue;
