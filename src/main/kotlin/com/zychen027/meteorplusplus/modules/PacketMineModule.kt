...
class PacketMineModule : Module(
    ...
    private val silent = sgGeneral.add(BoolSetting.Builder()
        .name("Silent")
        .description("静默发包（不播放动画/声音/粒子）")
        .defaultValue(false)
        .build())
    ...
    private fun sendStop() {
        ...
        // Delay 模式延迟切换回原槽位
        if (autoSwitch.get() == SwitchMode.Delay && hasSwitch) {
            switchTimer.reset()
        }
    ...
    /**
     * 静默发包核心方法
     * 使用 PendingUpdateManager 获取正确的序列号
     */
    private fun sendSequencedPacket(packetCreator: SequencedPacketCreator) {
        if (mc.networkHandler == null || mc.player == null || mc.world == null) return
        
        // 使用 PendingUpdateManager 获取正确的序列号
        val pendingUpdateManager = mc.world.pendingUpdateManager.incrementSequence()
        try {
